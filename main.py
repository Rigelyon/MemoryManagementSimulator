...
class MemoryManagementApp:
    ...
    def create_ui(self):
        ...
        name_entry = ctk.CTkEntry(process_frame, textvariable=self.process_name_var, width=150)
        ...
        list_label = ctk.CTkLabel(process_list_frame, text="Process Queue:", font=ctk.CTkFont(weight="bold"))
        ...
        clear_btn = ctk.CTkButton(self.left_panel, text="Clear All", command=self.clear_all, fg_color=SECONDARY_COLOR)
    ...
    def update_partition_ui(self, choice):
        ...
        for i in range(num_partitions):
            ...
            slider = ctk.CTkSlider(slider_frame, from_=5, to=100, number_of_steps=19,
                                   variable=percentage_var, command=lambda val, idx=i: self.on_slider_change(val, idx))
            slider.pack(side="left", fill="x", expand=True, padx=5)

            percentage_label = ctk.CTkLabel(slider_frame, text=f"{default_percentage:.1f}%", width=60)
    ...
    def on_slider_change(self, value, index):
        ...
        if abs(total - 100) > 0.1:
            excess = total - 100
            adjustable_sliders = [(i, var) for i, (var, _) in enumerate(self.active_sliders) 
        ...
        self.partition_values = [slider_var.get() for slider_var, _ in self.active_sliders]

    def apply_partitions(self):
        total = sum(self.partition_values)
        if abs(total - 100) > 0.1:
            self.partition_values = [val * (100 / total) for val in self.partition_values]
        ...
        if result:
            ...
            self.status_var.set(f"Memory partitioned into {len(self.partition_values)} sections")
        else:
    ...
    def add_to_process_list(self, process):
        ...
        partition_text = f" [Partition {partition_id+1}]" if partition_id is not None else ""

        header_frame = ctk.CTkFrame(process_item, fg_color="transparent")
        header_frame.grid(row=0, column=0, sticky="ew", padx=5, pady=2)

        process_name_label = ctk.CTkLabel(header_frame, text=f"{process.name} ({process.size} MB)", anchor="w")
        process_name_label.pack(side="left")

        if partition_id is not None:
            partition_colors = ["#3a7ebf", "#bf3a3a", "#3abf7e", "#7e3abf", "#bf7e3a", "#7ebf3a"]
            color = partition_colors[partition_id % len(partition_colors)]
            partition_label = ctk.CTkLabel(header_frame, text=f"P{partition_id}", 
                                            fg_color=color, text_color="white", 
                                            corner_radius=10, width=30, height=20)
        ...
        if process.algorithm:
            ...
            algo_letter = ''.join(word[0] for word in process.algorithm.split()) if process.algorithm else "?"
            algo_label = ctk.CTkLabel(header_frame, text=algo_letter, 
                                       fg_color=algo_color, text_color="white", 
                                       corner_radius=10, width=30, height=20)
            algo_label.pack(side="right", padx=5)

        time_var = ctk.StringVar(value=f"Time: {process.elapsed_time}s / {process.duration}s")
        time_label = ctk.CTkLabel(process_item, textvariable=time_var)
        time_label.grid(row=1, column=0, sticky="w", padx=5, pady=2)

        remove_btn = ctk.CTkButton(process_item, text="Remove", width=70, height=25,
    ...
    def update_process_times(self):
        for process_name, ui_data in self.process_ui_elements.items():
            ...
            if process_name in self.memory_manager.processes:
                ...
                time_var.set(f"Time: {process.elapsed_time}s / {process.duration}s ({remaining_time}s left)")
            else:
